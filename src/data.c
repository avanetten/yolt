...
data load_data_detection(int n, char **paths, int m, int w, int h, int c, int boxes, int classes, float jitter, float hue, float saturation, float exposure)//, int load_data_verbose)
    // AVE: c is number of bands
//data load_data_detection(int n, char **paths, int m, int w, int h, int boxes, int classes, float jitter, float hue, float saturation, float exposure)
{
	int load_data_verbose = 1;
    char **random_paths = get_random_paths(paths, n, m);
    int i;
    data d = {0};
    d.shallow = 0;
    ...
    for(i = 0; i < n; ++i){
		if (load_data_verbose==1){
			fprintf(stderr, "load image path: %s\n", random_paths[i]);
		}
        image orig = load_image(random_paths[i], 0, 0, c);  // AVE
        //image orig = load_image_color(random_paths[i], 0, 0);
        ...
        image sized = resize_image(cropped, w, h);
        if(flip) flip_image(sized);
        // random_distort_image uses rgb_to_hsv, which fails if number of channels != 3
        //random_distort_image(sized, hue, saturation, exposure);
        if (c==3){
            random_distort_image(sized, hue, saturation, exposure);
        }
...
void *load_thread(void *ptr)
{
    ...
    if (a.type == OLD_CLASSIFICATION_DATA){
        ...
        *a.d = load_data_region(a.n, a.paths, a.m, a.w, a.h, a.num_boxes, a.classes, a.jitter, a.hue, a.saturation, a.exposure);
		...
		// if training yolt, a.type == detection_data
        *a.d = load_data_detection(a.n, a.paths, a.m, a.w, a.h, a.c, a.num_boxes, a.classes, a.jitter, a.hue, a.saturation, a.exposure);//, a.load_data_verbose);
        ...
        *a.d = load_data_compare(a.n, a.paths, a.m, a.classes, a.w, a.h);
		...
		//fprintf(stderr, "a.c: %d\n", a.c);
        *(a.im) = load_image(a.path, 0, 0, a.c); // AVE
